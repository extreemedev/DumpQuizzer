...
// Carica il quiz selezionato
async function loadQuizFromFile(filename) {
    if (window.electronAPI && window.electronAPI.loadQuiz) {
        quizData = await window.electronAPI.loadQuiz(filename);
        document.getElementById('slider').max = quizData.questions.length;
    } else {
        // fallback: fetch locale (solo per sviluppo browser)
...
// Gestisci la selezione del quiz
const quizSelector = document.getElementById('quiz-selector');
if (quizSelector) {
    quizSelector.addEventListener('change', async function() {
        selectedQuizFile = this.value;
        await loadQuizFromFile(selectedQuizFile);
    });
}

// Carica il primo quiz all'avvio se presente
window.addEventListener('DOMContentLoaded', async () => {
    const selector = document.getElementById('quiz-selector');
    if (selector && selector.value) {
        selectedQuizFile = selector.value;
        await loadQuizFromFile(selectedQuizFile);
    }
});
